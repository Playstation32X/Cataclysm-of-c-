...
bool AreYoulookingAt(float targetX, float targetY, float targetZ)
{
	float dirX = cos((pry + 90) * rads);
	float dirZ = sin((pry + 90) * rads);
	float dirY = cos((prx + 90) * rads);
	float dx = targetX - px;
	float dy = targetY - py;
	float dz = targetZ - pz;
	float tolerance = 3;
	float dotProduct = dirX * dx + dirY * dy + dirZ * dz;
	return(dotProduct > 0 && abs(dx) < tolerance && abs(dy) < tolerance && abs(dz) < tolerance);
}
...
void animatedSprite(glm::vec3 pos,glm::vec3 scale,int frames,int id)
{
	...
	float dx =pos.x-px;
	float dz = pos.z-pz;
	float Angle = atan2(dx,dz)*(180/3.14159);
	...
	void ControlTheCharacters()
	{
		...
		{ 
			...
			static float increment = 0;
				increment+=0.01;
		 velocity.y =(sin(increment)*(rand()%4));
			animatedSprite(Pos, size, 4, cID);
		}
		break;
	    }
...
{   
	...
	for (auto& w : wallz)
	{
		...
		float DZ = w.WZ - bgz;
		float Dist = sqrt(DX*DX+DY*DY+DZ*DZ); 
		if (cubecollide(bgx, bgy, bgz, w.WX-w.scalex,w.WX+w.scalex, w.WY-3,w.WY+3, w.WZ-5,w.WZ+5))
		{
			bgx += DX / Dist;
			bgy += DY / Dist;
			bgz += DZ / Dist; 
	...
	if (tool==1)
	{
		...
		{ 
			for (auto& w : wallz)
			{
				...
				if (AreYoulookingAt(w.wx,w.wy,w.wz))
					w.active = false;
			}
			for (auto& t : tilez)
			{

				if (AreYoulookingAt(t.fx, t.fy, t.fz))
					t.active = false;
			}
...
void Initialize()
{ 
//	PlaySound(L"C:/Users/joemama23x/comtex/PRAISETHELORD.wav",NULL,SND_FILENAME);
	...
	texloader(); 
	CharacterLoader(glm::vec3(0,7,0),glm::vec3(7,7,7),glm::vec3(0,0,0),"aluminiumMan");
	CharacterLoader(glm::vec3(0, 7, 37), glm::vec3(7, 7, 7), glm::vec3(0, 0, 0), "aluminiumMan");
	CharacterLoader(glm::vec3(0, 7, 74), glm::vec3(7, 7, 7), glm::vec3(0, 0, 0), "aluminiumMan");
}
...
{ 
	...
	if (placeObject < 0||placeObject>2)
		placeObject = 0;
}
